class Iris:
    ...
    def execute(self):
        ...
        #print(df.columns)
        """
           IIndex(['sepal length (cm)', 'sepal width (cm)', 'petal length (cm)',
           'petal width (cm)', 'species', 'is_train'],
          dtype='object')
        """
        df['species'] = pd.Categorical.from_codes(iris.target, iris.target_names) #컬럼 추가
        #print(df.columns)
        ...
        #print(features)
        y = pd.factorize(train['species'])[0] #종을 숫자화??
        print(y)
        ...
        t = pd.crosstab(test['species'], preds, rownames=['Actual Species'],
                    colnames=['Predicted Species']) #colnames, rownames
        #print(t)
        #피쳐별 중요도
