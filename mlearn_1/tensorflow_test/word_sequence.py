class WordSequence:
    ...
    def execute():
        ...
        embedings = tf.Variable(tf.random_uniform([voc_size, embeding_size], -1.0, 1.0))
        #word2vec 모델의 결과 값인 임베딩 벡터를 저장할 변수
        # 총 단어의 갯수와 임베딩 갯수의 크기로 하는 두개의 차원()을 갖습니다.
        # embedding vector 의 차원에서 학습할 입력값에 대한 행동을 뽑아 옵니다.
        """
            embedding                       input                     selected
            [
             [1,2,3], ->    [2,3](2번째, 3번째 값을 뽑아 와라)   -> [[2,3,4],[3,4,5]]
             [2,3,4],
             [3,4,5],
             [4,5,6]
            ]
        ...
        nce_weight = tf.Variable(tf.random_uniform([voc_size, embeding_size], -1.0, 1.0)) #정규분포
